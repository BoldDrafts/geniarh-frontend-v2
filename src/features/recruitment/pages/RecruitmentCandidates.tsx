...
const RecruitmentCandidates: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<'createdAt' | 'updatedAt' | 'firstName' | 'lastName' | 'matchScore'>('matchScore');
  ...
  const updateCandidateEmail = async (recruitmentId: string, candidateId: string, newEmail: string) => {
    try {
      ...
      await fetchCandidates(pagination.page, selectedStatus, minMatchScore, maxMatchScore);
  ...
  useEffect(() => {
    if (id) {
      ...
      fetchCandidates(1, selectedStatus, minMatchScore, maxMatchScore);
    }
  }, [id, sortBy, sortOrder]);
  ...
  const handleStatusFilter = (status?: CandidateStatus) => {
    ...
    fetchCandidates(1, status, minMatchScore, maxMatchScore);
  };
  ...
  const handleMatchScoreFilter = (min?: number, max?: number) => {
    ...
    fetchCandidates(1, selectedStatus, min, max);
  };
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            onClick={() => navigate(`/recruitment/${id}/candidates/bulk-upload`)}
            disabled={loading.updating}
            className="flex items-center px-4 py-2 bg-white border border-blue-600 text-blue-600 rounded-md hover:bg-blue-50 transition-colors disabled:opacity-50"
          >
