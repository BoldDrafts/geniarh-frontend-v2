              ...
              >
                  ...
                  {interview.participants.length > 0 ? (
                    <div className="flex items-center">
                      <div className="flex -space-x-2">
                        {interview.participants.slice(0, 2).map((interviewer, index) => (
                          <div 
                            key={index} 
                      ...
                      {interview.participants.length > 2 && (
                        <span className="ml-1 text-xs text-gray-500">
                          +{interview.participants.length - 2} more
                        </span>
                      )}
                    </div>
                  ) : (
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(interview.status)}`}>
                    ...
                    {interview.status === 'Cancelled' && <AlertCircle className="h-3 w-3 mr-1" />}
                    {interview.status === 'InProgress' && <Clock className="h-3 w-3 mr-1" />}
                    {interview.status.charAt(0).toUpperCase() + interview.status.slice(1)}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <div className="flex space-x-3">
                    {(interview.status === 'Scheduled' || interview.status === 'InProgress') && (
                      <button
