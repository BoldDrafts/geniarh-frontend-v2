...
if [[ "create" == $ACTION ]] ; then
   ${script_dir}/ingress-sign.sh --profile ${P_BRANCH} --action delete
  ${KUBECTL_COMMAND} create secret tls "${APP_CONTAINER}-sign-tls" --namespace ${NAMESPACE} --key=${DIR_CERTS}/${DOMAIN}/privkey.pem --cert=${DIR_CERTS}/${DOMAIN}/fullchain.pem
fi

if [[ "delete" == $ACTION ]] ; then
  if [[ ! -z "$(${KUBECTL_COMMAND} get secret -n ${NAMESPACE} | grep "^${APP_CONTAINER}-sign-tls")" ]] ; then
    ${KUBECTL_COMMAND} delete secret -n ${NAMESPACE} "${APP_CONTAINER}-sign-tls"
  fi
fi